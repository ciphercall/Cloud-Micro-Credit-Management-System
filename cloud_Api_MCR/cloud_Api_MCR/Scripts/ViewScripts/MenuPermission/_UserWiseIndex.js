...
MenuPermissionApp.controller("ApiRoleController", function ($scope, $http) {
    ...
    $scope.search = function (event) {
        ...
        var uid = $('#txtUid').val();
        var tid = $('#txtTid').val();
        var selectUserId = $('#ddlUserId option:selected').val();
        var moduleid = $('#ddlModuleId option:selected').val();
        var menutypeid = $('#ddlMenuTypeId option:selected').val();
        $http.get('/api/Role/UserWisePermissionList/', {
            params: {
                userCompanycode: cid,
                usercode: uid,
                selectUserCode: selectUserId,
                moduleId: moduleid,
                menuType: menutypeid
            },
            headers: {
    ...
    $scope.toggleEdit_Cancel = function () {
        ...
        var uid1 = $('#txtUid').val();
        var tid1 = $('#txtTid').val();
        var selectUserId1 = $('#ddlUserId option:selected').val();
    ...
    $scope.update = function () {
        ...
        this.testitem.SelectUserId = $('#ddlUserId option:selected').val();
        ...
        if (this.testitem.SelectCompId !== "" && this.testitem.Moduleid !== "" && this.testitem.Menutp !== "") {
            $http.post('/api/Role/UserWisePermissionUpdate', this.testitem, config).success(function (data) {
                alert(data.message);
                ...
                var uid3 = $('#txtUid').val();
                var tid3 = $('#txtTid').val();
                var selectUserId3 = $('#ddlUserId option:selected').val();
