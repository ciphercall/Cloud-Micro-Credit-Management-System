...
{
    public class McrProcessController : AppController
    {
        Int64 _loggedCompid, _loggedUserid;
        String _loggedToken, _loggedUserTp;

        public McrProcessController()
        {
                        ...
                        using (HttpContent content = response.Content)
                        {
                            ...
                            if (success == true)
                            {
                                ...
                                foreach (JToken get in results)
                                {
                                    ...
                                    if (getModel.Compid == _loggedCompid && getModel.Userid == _loggedUserid && getModel.Controllername == "McrProcess" && getModel.Actionname == "Index")
                                    {
        ...
        public ActionResult IndexPost(ViewGlMaster passmodel)
        {
            if (passmodel.Transdt != null)
            {
                ...
                String mcrProcessUrl = HttpClientBaseAddress.BaseAddress() + "api/McrProcess/Process?userCompanycode=" + _loggedCompid +
                                          "&usercode=" + _loggedUserid;
                ...
                {
                    httpclient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Token", _loggedToken);
                    using (HttpResponseMessage response = httpclient.PostAsync(mcrProcessUrl, query).Result)
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            using (HttpContent content = response.Content)
                            {
                                ...
                                {
                                    TempData["McrProcess_Date"] = message;
                                    return View("Index");
                                }
                            }
                        }
